# This script borrow from
# https://github.com/STVIR/pysot/blob/master/pysot/datasets/augmentation.py
import numpy as np
import cv2
from utils.coordinate import corner2center, Center, center2corner, Corner
...
class Augmentation:
    ...
    def _shift_scale_aug(self, image, bbox, crop_bbox, size):
        ...
        if self.shift:
            ...
            sx = max(-x1, min(im_w - 1 - x2, sx))  # 左右两边不要超出
            sy = max(-y1, min(im_h - 1 - y2, sy))  # 上下两边不要超出
